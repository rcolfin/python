...
SOCIAL_MEDIA_CONNECTIONS_URL="https://gist.githubusercontent.com/rcolfin/65515e06f7a9df787eb8684c0412a746/raw/911f098ce8fa55f4eaec0634805b0fb3d353f510/social_media_connections.txt"
...
function download_data_files() {
    ...
    download_file "${SOCIAL_MEDIA_CONNECTIONS_URL}"
}
...
function run_map_reduce_package() {
    ...
    log INFO docker exec namenode hadoop jar /opt/hadoop-3.2.1/share/hadoop/tools/lib/hadoop-streaming-3.2.1.jar -file "mapper.py" -mapper "/opt/conda/bin/python mapper.py" -file "reducer.py"  -reducer "/opt/conda/bin/python reducer.py" -input "input/${DATAFILE}" -output output
    docker exec -w "${LOCAL_DOCKER_PACKAGE_PATH}/hadoop/${PACKAGE}" namenode hadoop jar /opt/hadoop-3.2.1/share/hadoop/tools/lib/hadoop-streaming-3.2.1.jar -file "mapper.py" -mapper "/opt/conda/bin/python mapper.py" -file "reducer.py"  -reducer "/opt/conda/bin/python reducer.py" -input "input/${DATAFILE}" -output output
...
function run_map_reduce_social_media_connect() {
    run_map_reduce_package "social_media_connect" "$(basename "${SOCIAL_MEDIA_CONNECTIONS_URL}")"
}
