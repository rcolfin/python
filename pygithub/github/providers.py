...
class Application(TokenProvider, HttpClient):
    ...
    def access_token(self) -> str:
        ...
        if self._access_token is None or self._access_token[1] <= time.time():
            self._access_token = self._query_access_token()
    ...
    def get_app_installations(self) -> Iterable[dict[str, Any]]:
        jwt_token = self.generate_jwt()
    ...
    def _query_access_token(self) -> tuple[str, int]:
        ...
        token = next(cast(str, r["token"]) for r in responses)
